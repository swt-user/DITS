...
if __name__ == "__main__":
    ...
    for model, model_path in model_paths:
        ...
        for i in range(args.num_gpu):
            ...
            CUDA_VISIBLE_DEVICES={args.device+i} vllm serve {model_path} --enable-lora --host 0.0.0.0 --port {args.port+i} --served-model-name "Llama-3" --enable-prefix-caching --max-lora-rank 64 --max-model-len 8096
            """,
                shell=True,
            )
        ...
        for i in range(args.num_gpu):
            os.system(
                f"""pkill -f "vllm serve {model_path} --enable-lora --host 0.0.0.0 --port {args.port+i} --served-model-name Llama-3 --enable-prefix-caching --max-lora-rank 64 --max-model-len 8096" """
            )
        time.sleep(5)
        print("pkill finish!!!!!")
