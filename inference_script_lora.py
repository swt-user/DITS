...
if __name__ == "__main__":
    ...
    for model, model_path in model_paths:
        ...
        for i in range(args.num_gpu):
            ...
            CUDA_VISIBLE_DEVICES={args.device+i} vllm serve {model_path} --enable-lora --lora-modules sql_lora={args.model_lora_path} --host 0.0.0.0 --port {args.port+i} --served-model-name "Llama-3" --enable-prefix-caching --max-lora-rank 64 --dtype float16 --lora-dtype float16 --max-seq-len-to-capture 2000
            """,
                shell=True,
            )
