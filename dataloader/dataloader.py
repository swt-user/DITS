...
class DataloaderForHotpotQA(BaseDataloader):
    ...
    def __init__(
        self, dataset: str = hotpot_qa_path, name: str = "distractor", split: str = "train", current_id=0,
    ):
...
class DataloaderForMWHQA(BaseDataloader):
    def __init__(
        ...
        with open(os.path.join("/home/wentaos/.cache/huggingface/datasets/MWHQA/data", f"{split}.json"), "r") as f:
...
class DataloaderForTrivalQA(BaseDataloader):
    def __init__(
        ...
        self.split = split
        self.train_set = load_dataset(
            # path="/data/user_data/wentaos/Optima/original_datasets/mandarjoshi___trivia_qa/rc/0.0.0/0f7faf33a3908546c6fd5b73a660e0f8ff173c2f",
...
def process_dataloader_for_sft(
        ...
        if not is_mix:
            for line in tqdm(f):
                ...
                if reward_list[index] < episilon:
                    print(results[index]["correct_score"])
                    continue
                count += 1
    ...
    # train_dict = {"messages": messages[:len(messages)-2]}
    # test_dict = {"messages": messages[len(messages)-2:]}
