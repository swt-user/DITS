...
def get_score_deploy(
    ...
    # Log the input parameters to a file
    # log_entry = {
    #     "timestamp": datetime.now().isoformat(),
    #     "parameters": {
    #         "result": result,
    #         "max_token_count": max_token_count,
    #         "tokenizer": str(tokenizer),  # Convert tokenizer to string representation
    #         "model_url": model_url,
    #         "cal_ppl": cal_ppl,
    #         "_lambda1": _lambda1,
    #         "_lambda2": _lambda2,
    ...
    try:
        ...
        # log_entry["error"] = f"错误类型：{exc_type.__name__}\n错误信息：{exc_value}\n堆栈信息（可用 traceback 进一步处理）：{exc_traceback}"
        print(f"错误类型：{exc_type.__name__}")
        print(f"错误信息：{exc_value}")
        print("堆栈信息（可用 traceback 进一步处理）：", exc_traceback)
        correct_score = 0
    token_score = _lambda1 * result["token_count"] / max_token_count
    if "large" in result["conversation"]:
        token_score = -1
    ...
    # log_entry["result"] = f"correct: {correct_score},token: {token_score},ppl:{ppl_score},reward:{correct_score + token_score + ppl_score}"
