...
def collect_data_influence(gpu_id, i, task_queue, lock, interval):
    ...
    while True:
        ...
        #     accelerate launch --config_file YOUR_HOME_PATH_HERE/alignment-handbook/recipes/accelerate_configs/multi_gpu.yaml --num_processes=1 --main_process_port {main_processs_port} \
        #     YOUR_HOME_PATH_HERE/alignment-handbook/scripts/run_dpo_collect_noserver.py \
        #     YOUR_HOME_PATH_HERE/alignment-handbook/recipes/Llama3-8B-multi-DI/{dataset_type}_sft_dpo/dpo_iteration_{i}_lora_collect_warmup.yaml \
        #     --load_in_4bit=false \
        ...
        inference_command = [
            ...
            --model_root_path YOUR_FILE_PATH_HERE/checkpoints/{dataset_type}_sft_dpo_DI/sft_iteration_{i} \
            --model_name iteration_0 \
            --model_lora_path YOUR_FILE_PATH_HERE/checkpoints/{dataset_type}_sft_dpo_DI_lora/dpo_iteration_{i} \
            ...
            --end_id {end_point} \
            --output_root_path "YOUR_FILE_PATH_HERE/inference_results/{dataset_type}_dpo_iteration_{i}_server_stepnum5/id" \
        ...
        for id in range(start_point, end_point):
            checkpoint_path = f"YOUR_FILE_PATH_HERE/checkpoints/{dataset_type}_sft_dpo_DI_lora/dpo_iteration_{i}_id_{id}"
...
def sft_dpo_train_v2_DI(
    ...
    for i in range(iteration_times):
        ...
        data_influence_path = f"YOUR_FILE_PATH_HERE/inference_results/{dataset_type}_dpo_iteration_{i}_server_stepnum5/"
        ...
        for t in range(base_num, len(train_dataset)//5, interval):
            ...
            process = subprocess.Popen(
                ...
                cd YOUR_HOME_PATH_HERE/alignment-handbook &&\
                ACCELERATE_LOG_LEVEL=info NCCL_P2P_DISABLE=1 accelerate launch --config_file YOUR_HOME_PATH_HERE/alignment-handbook/recipes/accelerate_configs/multi_gpu.yaml --num_processes=1 --main_process_port {main_processs_port} YOUR_HOME_PATH_HERE/alignment-handbook/scripts/run_dpo_collect_noserver.py YOUR_HOME_PATH_HERE/alignment-handbook/recipes/Llama3-8B-multi-DI/{dataset_type}_sft_dpo/dpo_iteration_{i}_lora_collect_warmup.yaml --load_in_4bit=false --start_point={start_point} --end_point={end_point} --step_num=5
                """, 
                shell=True)
            process.wait()
            time.sleep(10)
            ...
            #                 --model_root_path YOUR_FILE_PATH_HERE/optima-checkpoints/${dataset_type}_sft/ \
            #                 --model_lora_path YOUR_FILE_PATH_HERE/optima-checkpoints/${dataset_type}_dpo_lora/dpo_iteration_0 \
            #                 --tokenizer_path YOUR_MODEL_PATH_HERE \
            #                 --device 0 \
            #                 --port {num_port} \
            #                 --dataset_type ${dataset_type} \
            ...
            #             do \
            #                 rm -r YOUR_FILE_PATH_HERE/optima-checkpoints/${dataset_type}_dpo_lora/dpo_iteration_0_id_${id}\
            #             done\
            #             """,
            #             shell=True,
            #         )
