...
def sft_dpo_train_v2(
    ...
    for i in range(iteration_times):
        ...
        if os.path.exists(dataset_path):
            ...
            process = subprocess.Popen(
                f"""source ~/.bashrc && \
                    ...
                    python ppl_deploy.py --model_path {initial_model_path}
                    """,
                shell=True,
            )
        ...
        try:
            with open(
                os.path.join(mid_yaml_root_path, f"sft_iteration_{i}_ready.yaml"), "r"
            ) as f:
                pass
        except:
            with open(
            ...
            os.rename(os.path.join(mid_yaml_root_path, f"sft_iteration_{i}.yaml"), os.path.join(mid_yaml_root_path, f"sft_iteration_{i}_ready.yaml"))
        ...
        try:
            with open(
                os.path.join(mid_yaml_root_path, f"dpo_iteration_{i}_ready.yaml"), "r"
            ) as f:
                pass
        except:
            with open(
            ...
            process = subprocess.Popen(
                f"""source ~/.bashrc &&\
                conda activate {alignment_env} && \
                ACCELERATE_LOG_LEVEL=info NCCL_P2P_DISABLE=1 accelerate launch --config_file recipes/accelerate_configs/deepspeed_zero3.yaml scripts/run_dpo.py  {os.path.join(mid_yaml_root_path,f"dpo_iteration_{i}.yaml")} """,
                shell=True,
                cwd="./alignment-handbook",
            )
            process.wait()
            os.rename(os.path.join(mid_yaml_root_path, f"dpo_iteration_{i}.yaml"), os.path.join(mid_yaml_root_path, f"dpo_iteration_{i}_ready.yaml"))
    ...
    for i in range(iteration_times):
        ...
        try:
            ...
            process = subprocess.Popen(
                f"""source ~/.bashrc &&\
                conda activate {alignment_env} && \
                ACCELERATE_LOG_LEVEL=info NCCL_P2P_DISABLE=1 accelerate launch --config_file recipes/accelerate_configs/multi_gpu.yaml scripts/run_sft.py  {os.path.join(mid_yaml_root_path,f"sft_iteration_{i}.yaml")} """,
                shell=True,
                cwd="./alignment-handbook",
            )
            process.wait()
