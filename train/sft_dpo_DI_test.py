...
import sys
import os
sys.path.append("YOUR_HOME_PATH_HERE/local_influence")
from generate_influence_dataset import generate_influence_dataset

def collect_data_influence_once(gpu_id, mid_yaml_root_path, check_point_root_path, data_influence_root_path, t, i, interval, dataset_type):
    ...
    process = subprocess.Popen(
        ...
        cd YOUR_HOME_PATH_HERE/alignment-handbook &&\
        CUDA_VISIBLE_DEVICES={gpu_id} ACCELERATE_LOG_LEVEL=info NCCL_P2P_DISABLE=1 accelerate launch --config_file YOUR_HOME_PATH_HERE/alignment-handbook/recipes/accelerate_configs/multi_gpu.yaml --num_processes=1 --main_process_port {main_processs_port} YOUR_HOME_PATH_HERE/alignment-handbook/scripts/run_dpo_collect_noserver.py {mid_yaml_root_path}/dpo_iteration_{i}_lora_collect_warmup.yaml --load_in_4bit=false --start_point={start_point} --end_point={end_point} --step_num=5
        """, 
        shell=True)
    process.wait()
    time.sleep(10)
    ...
    try: 
        ...
        process = subprocess.Popen(
            ...
            cd YOUR_HOME_PATH_HERE && \
            export VLLM_ALLOW_RUNTIME_LORA_UPDATING=True && \
            OUTLINES_CACHE_DIR=./outlines/{dataset_type}/{gpu_id} python inference_script_server_train_dynamic.py \
            --model_root_path {check_point_root_path}/sft_iteration_{i} \
            --model_name iteration_{i} \
            --model_lora_path {check_point_root_path}_lora/dpo_iteration_{i} \
            --tokenizer_path {os.environ["INITIAL_MODEL_PATH"]} \
            --device {gpu_id} \
            --port {process_port} \
            ...
            --end_id {end_point} \
            --output_root_path {data_influence_root_path}/dpo_iteration_{i}_server_stepnum5/id \
    ...
    for id in range(start_point, end_point):
        checkpoint_path = f"{check_point_root_path}_lora/dpo_iteration_{i}_id_{id}"
...
# 调度任务
def schedule_tasks(args_list, available_gpus, mid_yaml_root_path, check_point_root_path, data_influence_root_path):
        ...
        while args_list or futures_to_gpu:
            ...
            for gpu_id, is_free in gpu_status.items():
                if is_free and args_list:
                    ...
                    future = executor.submit(collect_data_influence_once, gpu_id, mid_yaml_root_path, check_point_root_path, data_influence_root_path,  *task_args)
...
def sft_dpo_train_v2_DI(
    ...
    for i in range(iteration_times):
        ...
        config["model_name_or_path"] = initial_model_path
        config["tokenizer_name_or_path"] = initial_model_path
        config["dataset_mixer"] = {dataset_path: 1.0}
        config["output_dir"] = os.path.join(check_point_root_path, f"sft_iteration_{i}")
        ...
        try:
            ...
            process = subprocess.Popen(
                f"""source ~/.bashrc &&\
                conda activate {alignment_env} && \
                ACCELERATE_LOG_LEVEL=info NCCL_P2P_DISABLE=1 accelerate launch --config_file recipes/accelerate_configs/deepspeed_zero3.yaml scripts/run_sft.py  {os.path.join(mid_yaml_root_path,f"sft_iteration_{i}.yaml")} """,
                shell=True,
        ...
        if os.path.exists(dataset_path):
            ...
            process = subprocess.Popen(
                ...
                source ~/.bashrc && \
                conda activate {alignment_env} && \
                HF_DATASETS_CACHE="../huggingface_cache/huggingface_dataset_cache" python YOUR_HOME_PATH_HERE/reward_main.py --cleaned_output_path {os.path.join(mid_dpo_jsonl_root_path,f"iteration_{i}_dpo_format.jsonl")}\
        ...
        try:
            ...
            process = subprocess.Popen(
                f"""source ~/.bashrc &&\
                conda activate {alignment_env} && \
                ACCELERATE_LOG_LEVEL=info NCCL_P2P_DISABLE=1 accelerate launch --config_file recipes/accelerate_configs/multi_gpu.yaml --num_processes=1 scripts/run_dpo.py  {os.path.join(mid_yaml_root_path,f"dpo_iteration_{i}_lora.yaml")} --load_in_4bit=false""",
                shell=True,
                cwd="./alignment-handbook",
            )
            process.wait()
        ...
        collect_dataset_path = os.path.join(mid_dpo_dataset_root_path, f"{dataset_type}_dpo_mix_warmup_{len(train_dataset)//2}/iteration_{i}")
        
        if os.path.exists(collect_dataset_path):
            pass
        else: 
            data_influence_path = f"{data_influence_root_path}/dpo_iteration_{i}_server_stepnum5/"
            ...
            for t in range(base_num, len(train_dataset)//5, interval):
                args_list.append((t, i, interval, dataset_type))
            
            available_gpus = list(range(num_device))
            if args_list:
                schedule_tasks(args_list, available_gpus, mid_yaml_root_path, check_point_root_path, data_influence_root_path)
        ...
        config["model_name_or_path"] = model_path
        config["tokenizer_name_or_path"] = initial_model_path
        config["dataset_mixer"] = {collect_dataset_path: 1.0}
        config["output_dir"] = os.path.join(check_point_root_path, f"dpo_iteration_{i}"+ f"_mix_warmup_{len(train_dataset)//2}")

        try:
            with open(
                os.path.join(mid_yaml_root_path, f"dpo_iteration_{i}" +f"_mix_warmup_{len(train_dataset)//2}" + ".yaml"), "r"
            ) as f:
                pass
        except:
            with open(
                os.path.join(mid_yaml_root_path, f"dpo_iteration_{i}" +f"_mix_warmup_{len(train_dataset)//2}" + ".yaml" ), "w"
            ) as fout:
                fout.write(yaml.safe_dump(config))
            process = subprocess.Popen(
                f"""source ~/.bashrc &&\
                conda activate {alignment_env} && \
                ACCELERATE_LOG_LEVEL=info NCCL_P2P_DISABLE=1 accelerate launch --config_file recipes/accelerate_configs/deepspeed_zero3.yaml scripts/run_dpo.py  {os.path.join(mid_yaml_root_path, f"dpo_iteration_{i}" +f"_mix_warmup_{len(train_dataset)//2}" + ".yaml" )} """,
                shell=True,
                cwd="./alignment-handbook",
            )
            process.wait()
        model_path = os.path.join(check_point_root_path, f"dpo_iteration_{i}"+ f"_mix_warmup_{len(train_dataset)//2}")
