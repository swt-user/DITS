...
def collect_data_influence_once(gpu_id, t, i, interval, dataset_type):
    ...
    for i in range(iteration_times):
        ...
        try:
            ...
            process = subprocess.Popen(
                f"""source ~/.bashrc &&\
                conda activate {alignment_env} && \
                ACCELERATE_LOG_LEVEL=info NCCL_P2P_DISABLE=1 accelerate launch --config_file recipes/accelerate_configs/deepspeed_zero3.yaml scripts/run_dpo.py  {os.path.join(mid_yaml_root_path[21:], f"dpo_iteration_{i}" +f"_mix_warmup_{len(train_dataset)//2}" + ".yaml" )} """,
                shell=True,
                cwd="./alignment-handbook",
            )
            process.wait()
