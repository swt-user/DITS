...
from agent.agent import Agent, VllmAgent, BaseAgent, VLLMAgentNoServer, VLLMAgentNoServerMultiProcess
...
def vllm_data_generate_noserver_multiprocess(
    model_first: str,
    model_second: str,
    device_first: str,
    device_second: str,
    ...
    model_first = LLM(
        model = model_first, enable_lora=True, max_lora_rank=32,
    )
    ...
    if "Llama" in os.environ["INITIAL_MODEL_PATH"]: 
        ...
        chat_template = """{% set loop_messages = messages %}{% for message in loop_messages %}{% set content = '<|im_start|>' + message['role']+ '\n'+ message['content'] | trim + '<|im_end|>' %}{% if loop.index0 == 0 %}{% set content = content %}{% endif %}{{ content }}{% endfor %}{% if add_generation_prompt %}{{ '<|im_start|>assistant\n' }}{% endif %}{%- if is_alice %}\n    {{- 'Alice:' }}\n{%- endif %}\n{%- if is_bob %}\n    {{- 'Bob:' }}\n{%- endif %}"""
    ...
    agent_first = VLLMAgentNoServerMultiProcess(
        llm=llm_first,
        system_prompt=llmMessage(role="system", content=prompt),
        name="Alice",
        prompt_template=prompt,
    ...
    agent_second = VLLMAgentNoServerMultiProcess(
        llm=llm_second,
        system_prompt=llmMessage(role="system", content=prompt),
        name="Bob",
        prompt_template=prompt,
    ...
    for i in range(0, sample_count):
        if dataloader.data_type != "mix":
            ...
            if data_type == "qa":
                the_prompt = prompt
                prompt_pool_path = "YOUR_PATH_HERE"
            elif data_type == "math":
                the_prompt = prompt_multi_math
                no_use_prompt_pool = True
                is_debate = True
    ...
    for idx, (
        ...
        if current_threads >= thread_max_count:
            while not event.is_set():
                # print("aaaaaaaaaaaaa")
                if thread_count[0] >= thread_max_count:
                    # 主线程汇总结果并提交
                    intermediate_results = []
                    while not result_queue.empty():
                        intermediate_results.append(result_queue.get())
                    sorted_list = sorted(intermediate_results, key=lambda x: x[0])
                    # 只保留元素1
                    message_input = [item[1] for item in sorted_list]
                    # 构造dict，完成元素0到最后index的映射
                    index_mapping = {i:item[0]  for i, item in enumerate(sorted_list)}

                    print(message_input)
                    return
                    results = model_first.generate(
                            message_input,
                            sampling_params,
                            lora_request=LoRARequest("lora", 1, lora_first_path),
                    )
                    ...
                    with lock:
                        for i, result in enumerate(results):
                            final_results[index_mapping[i]] = result
                        thread_count[0] = 0
                        #print(final_results)
                        #return
            ...
            for t in threads:
                if not t.is_alive():
                    print(f"Thread {t} has finished")
                    threads.remove(t)
                    current_threads -= 1
            event.clear()
        
        thread = threading.Thread(target=vllm_data_generate_once, args=(
            task_id,
            explore_count,
            agent_first,
            agent_second,
...
def vllm_data_generate_once(
    ...
    for j in range(explore_count):
        ...
        response_list, final_answer = conversation(
            agent_list, tokenizer_first, question, data_type, lock, result_queue, final_results, thread_count
        )
        print(
            f"...........................A:{final_answer}....................................."
        )
...
def conversation(agent_list: List[BaseAgent], tokenizer, question, data_type="qa", lock= None, result_queue=[], final_results=[], thread_count=[]):
    final_answer = ""
    current_agent_id = 0
    response_list = []
    now_round = 0
    while True:
        ...
        print("AAAAAAAAAAA")
        response = agent.step(lock, result_queue, final_results, thread_count)
        print(response)
