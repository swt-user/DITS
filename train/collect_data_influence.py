...
def collect_data_influence_once(gpu_id, t, i, interval, dataset_type, name):
    ...
    try: 
        ...
        process = subprocess.Popen(
            ...
            --model_root_path YOUR_FILE_PATH_HERE/checkpoints/{dataset_type}_sft_dpo_scale/sft_iteration_{i} \
            --model_name iteration_{i} \
            --model_lora_path YOUR_FILE_PATH_HERE/checkpoints/{dataset_type}_sft_dpo_scale/dpo_iteration_{i}_{name}_lora/dpo_iteration_{i} \
            ...
            --end_id {end_point} \
            --output_root_path "YOUR_FILE_PATH_HERE/inference_results/{dataset_type}_dpo_iteration_{i}_{name}_server_stepnum5/id" \
    ...
    for id in range(start_point, end_point):
        checkpoint_path = f"YOUR_FILE_PATH_HERE/checkpoints/{dataset_type}_sft_dpo_scale/dpo_iteration_{i}_{name}_lora/dpo_iteration_{i}_id_{id}"
...
if __name__ == "__main__":
    ...
    data_influence_path = f"YOUR_FILE_PATH_HERE/inference_results/{args.dataset_type}_dpo_iteration_{args.iteration}_{args.name}_server_stepnum5/"
    ...
    origin_dpo_yaml_path_lora = f"YOUR_HOME_PATH_HERE/alignment-handbook/recipes/Llama3-8B-multi-scale/{args.dataset_type}_sft_dpo/dpo/base_lora.yaml"
    mid_yaml_root_path = f"YOUR_HOME_PATH_HERE/alignment-handbook/recipes/Llama3-8B-multi-scale/{args.dataset_type}_sft_dpo"
    dataset_path = f"YOUR_FILE_PATH_HERE/my_datasets/{args.dataset_type}_sft_dpo_scale/dpo_{args.name}/iteration_{args.iteration}"
    check_point_root_path = f"YOUR_FILE_PATH_HERE/checkpoints/{args.dataset_type}_sft_dpo_scale/dpo_iteration_{args.iteration}_{args.name}_lora/dpo_iteration_{args.iteration}"
    # dpo warmup  train     
    ...
    if args.lora_path == None:
        config["model_name_or_path"] = os.path.join(f"YOUR_FILE_PATH_HERE/checkpoints/{args.dataset_type}_sft_dpo_scale/dpo_iteration_{args.iteration}_scale" + "_lora", "checkpoint-50")
    else:
    ...
    data_influence_path = f"YOUR_FILE_PATH_HERE/inference_results/{args.dataset_type}_dpo_iteration_{args.iteration}_{args.name}_server_stepnum5/"
    mid_dpo_jsonl_root_path = f"YOUR_FILE_PATH_HERE/results/{args.dataset_type}_sft_dpo_scale/dpo_{args.name}/"
    mid_dpo_dataset_root_path = f"YOUR_FILE_PATH_HERE/my_datasets/{args.dataset_type}_sft_dpo_scale/dpo_{args.name}/"
