...
class NewArguments:
    ...
    end_point: int = field(default=300, metadata={"help": "Number of processes to use for distributed training"})
    eval_num: int = field(default=1559, metadata={"help": "Local rank for distributed training"})
    step_num: int = field(default=1, metadata={"help": "Local rank for distributed training"})
...
def main():
    ...
    # if not os.path.exists(f"/home/wentaos/Optima/local_influence/noserver_test_{new_args.name_str}_{new_args.eval_num}"):
    #     os.mkdir(f"/home/wentaos/Optima/local_influence/noserver_test_{new_args.name_str}_{new_args.eval_num}")

    # try:
    #     with open(f"/home/wentaos/Optima/local_influence/noserver_test_{new_args.name_str}_{new_args.eval_num}/{new_args.start_point}.txt", "r") as file:
    ...
    end_point = min(len(raw_datasets["train"])//new_args.step_num, new_args.end_point)
    for i in range(new_args.start_point, end_point):

        if i in seen_ids:
            continue
        temp = [k for k in range(i*new_args.step_num, (i+1)*new_args.step_num)]
        ...
        trainer.save_model(training_args.output_dir+f"_id_{i}")
        logger.info(f"Model saved to {training_args.output_dir+f'_id_{i}'}")
        ...
        if trainer.accelerator.is_main_process:
            ...
            trainer.model.config.save_pretrained(training_args.output_dir+f"_id_{i}")
            ...
            # with open(f"/home/wentaos/Optima/local_influence/noserver_test_{new_args.eval_num}_{new_args.start_point}.txt", 'a', encoding='utf-8') as file:
            #     # 确保内容以换行符结束
