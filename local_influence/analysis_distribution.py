...
def Analysis_distribution(
    ...
    df = pd.DataFrame(columns=['id', 'best_100', "chosen_value", "distance"])
    ...
    for item in os.listdir(data_influence_path):
        ...
        if os.path.isdir(os.path.join(data_influence_path, item)):
            ...
            if os.path.exists(item_path):
                ...
                # df.loc[len(df)] = ['\n'.join(chosen_shuffled[id_item*stepnum:(id_item+1)*stepnum]), '\n'.join(rejected_shuffled[id_item*stepnum:(id_item+1)*stepnum]), answer,  mean(chosen_value_shuffled[id_item*stepnum:(id_item+1)*stepnum]), mean(distance_shuffled[id_item*stepnum:(id_item+1)*stepnum])]

    df.to_csv(f'YOUR_HOME_PATH_HERE/local_influence/{dataset_type}{scale_name}.txt', sep='\t', index=False)
    # df.to_csv(f'YOUR_HOME_PATH_HERE/local_influence/{dataset_type}_record.txt', sep='\t', index=False)
    import pandas as pd
    ...
    plt.savefig(f"YOUR_HOME_PATH_HERE/local_influence/{dataset_type}{scale_name}_figure.png", bbox_inches='tight')

if __name__ == "__main__":
    dataset_type = "mwh_qa"
    i = 2
    Analysis_distribution(
        # origin_dpo_path = "YOUR_FILE_PATH_HERE/results/mwh_qa_sft_dpo_DI/dpo/iteration_0_dpo_format.jsonl",
        origin_dpo_path = f"YOUR_FILE_PATH_HERE/results/{dataset_type}_sft_dpo_DI/dpo/iteration_{i}_dpo_format.jsonl",
        # origin_dpo_path = "YOUR_FILE_PATH_HERE/results/cbt_dpo/iteration_0_dpo_format.jsonl",
        # origin_dpo_path = "YOUR_FILE_PATH_HERE/results/mwh_qa_sft_dpo_scale/dpo_3_4/iteration_0_dpo_format.jsonl",
        dataset_path = "str",
        data_influence_path = f"YOUR_FILE_PATH_HERE/inference_results/{dataset_type}_dpo_iteration_{i}_server_stepnum5",
        # data_influence_path = "YOUR_HOME_PATH_HERE/inference_results/cbt_dpo_server_stepnum5_validation",
        # data_influence_path = "YOUR_FILE_PATH_HERE/inference_results/mwh_qa_dpo_iteration_0_3_4_server_stepnum5",
